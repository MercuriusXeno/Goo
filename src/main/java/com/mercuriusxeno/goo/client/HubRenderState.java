...
public class HubRenderState extends BlockEntityRenderState {
    ...
    public final boolean[] hasCanister = new boolean[HubBlockEntity.MAX_CANISTERS];

    /** Per-slot top gasket presence (true if a choral gasket is installed on top). */
    public final boolean[] hasTopGasket = new boolean[HubBlockEntity.MAX_CANISTERS];

    /** Per-slot bottom gasket presence (true if a choral gasket is installed on bottom). */
    public final boolean[] hasBottomGasket = new boolean[HubBlockEntity.MAX_CANISTERS];
