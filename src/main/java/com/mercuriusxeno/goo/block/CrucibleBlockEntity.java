...
public class CrucibleBlockEntity extends BlockEntity implements IGasketHolder, IGooReservoir {
    ...
    private int ignitionSprayTicks = 0;
    ...
    private boolean hasMeltableItem() {
        if (meltingItem.isEmpty()) return false;
        return !PartiallyMeltedItem.isFullyMelted(meltingItem);
    }

    /** Returns true if the fuel rod has ticks remaining, or is a fresh vanilla blaze rod. */
    public boolean hasFuel() {
        if (fuelRod.isEmpty()) return false;
        if (fuelRod.is(Items.BLAZE_ROD)) return true;
        return !DepletedBlazeRodItem.isEmpty(fuelRod);
    }
    ...
    protected void loadAdditional(ValueInput input) {
        ...
        platformY = input.getFloatOr("PlatformY", PLAT_FLOOR);
        prevPlatformY = platformY;
        loadMeltingState(input);
        reservoir.loadFrom(input.read("Reservoir", GooContents.CODEC).orElse(GooContents.EMPTY));
    ...
    private void saveMeltingState(ValueOutput output) {
        if (!meltingItem.isEmpty()) {
            output.store("MeltingItem", ItemStack.CODEC, meltingItem);
        }
        if (!fuelRod.isEmpty()) {
            output.store("FuelRod", ItemStack.CODEC, fuelRod);
        }
    }

    /** Loads melting item and fuel rod stacks. */
    private void loadMeltingState(ValueInput input) {
        meltingItem = input.read("MeltingItem", ItemStack.CODEC).orElse(ItemStack.EMPTY);
        fuelRod = input.read("FuelRod", ItemStack.CODEC).orElse(ItemStack.EMPTY);
    }

    /** Saves gasket UUID and partner reference. */
    private void saveGasketFields(ValueOutput output) {
        if (gasketId != null) {
            output.store("GasketId", UUIDUtil.STRING_CODEC, gasketId);
        }
        if (gasketPartner != null) {
            output.store("GasketPartner", GasketPartner.CODEC, gasketPartner);
        }
    }

    /** Loads gasket UUID and partner reference. */
    private void loadGasketFields(ValueInput input) {
        gasketId = input.read("GasketId", UUIDUtil.STRING_CODEC).orElse(null);
        gasketPartner = input.read("GasketPartner", GasketPartner.CODEC).orElse(null);
    }
