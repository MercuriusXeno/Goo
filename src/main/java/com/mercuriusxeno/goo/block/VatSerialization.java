 ...
 * Extracted to keep the block entity under the TooManyMethods threshold
 * while preserving framework overrides in VatBlockEntity itself.
 */
final class VatSerialization {
    ...
    private static void saveContentsAndLabel(VatBlockEntity be, ValueOutput output) {
        ...
        if (!contents.isEmpty()) { output.store(TAG_CONTENTS, GooContents.CODEC, contents); }
        if (be.label != null) { output.putString(TAG_LABEL, be.label); }
    }
    ...
    private static void saveStreamFields(VatBlockEntity be, ValueOutput output) {
        if (be.vatStreamType == null) { return; }
