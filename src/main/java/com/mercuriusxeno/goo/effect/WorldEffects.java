...
public final class WorldEffects {
    ...
    private static Map<GooType, WorldEffect> buildRegistry() {
        return new EnumMap<>(Map.ofEntries(
            entry(GooType.ROCK, new RockEffect()),     entry(GooType.BLAZE, new BlazeEffect()),
            entry(GooType.FROST, new FrostEffect()),   entry(GooType.METAL, new MetalEffect()),
            entry(GooType.CRYSTAL, new CrystalEffect()), entry(GooType.HEX, new HexEffect()),
