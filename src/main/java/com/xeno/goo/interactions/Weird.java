...
public class Weird
{
    ...
    {
        // NOOP TODO
        //GooInteractions.registerBlobHit(fluidSupplier.get(), "weird_transport", Weird::weirdTransport, (c) -> true);
...
//    private static boolean weirdTransport(BlobHitContext splatContext) {
//        int warpCost = GooMod.config.costOfBlobInteraction(splatContext.fluid(), splatContext.interactionKey());
//        List<GooBlobController> nearbyWarpBlobHit = splatContext.world().getEntitiesWithinAABB(GooBlobHit.class,
//                splatContext.splat().getBoundingBox().grow(BOUNDS_REACH),
//                    (e) -> !e.equals(splatContext.splat())
//                        && GooBlobHit.getGoo(e).getFluidInTankInternal(0).getFluid().equals(Registry.WEIRD_GOO.get())
//                        && GooBlobHit.getGoo(e).getFluidInTankInternal(0).getAmount() >= warpCost
//                        && e.cooldown() <= 0
//        );
//        if (nearbyWarpBlobHit.size() == 0) {
//            return false;
//        }
//        nearbyWarpBlobHit.sort(Comparator.comparingDouble(c -> c.getDistance(splatContext.splat())));
//        GooBlobHit nearestWarp = nearbyWarpBlobHit.get(0);
...
//                GooBlobHit.getGoo(nearestWarp).drain(warpCost, IFluidHandler.FluidAction.EXECUTE);
...
//    }
//
//    private static void doEffects(GooBlobHit splat, boolean isPlayingSound) {
...
//
//    private static Tuple<Boolean, Vector3d> testSafePassage(BlobHitContext splatContext, GooBlobHit nearestWarp, Entity entity) {
