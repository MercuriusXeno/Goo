...
public class BlobHitEntityInteractionPacket implements IGooModPacket {
	private int entityHitId;
	private int ownerId;
	private int blobId;

	public BlobHitEntityInteractionPacket(LivingEntity entityHit, LivingEntity owner, HexController blob) {
	...
	public BlobHitEntityInteractionPacket(PacketBuffer buf) {
		read(buf);
	}
		...
		supplier.get().enqueueWork(new Runnable() {
			...
			public void run() {
				...
				if (supplier.get().getDirection().getReceptionSide() == LogicalSide.CLIENT) {
					...
					GooInteractions.tryResolving((LivingEntity) e, (LivingEntity) owner, (HexController) blob);
				}
			}
		});
