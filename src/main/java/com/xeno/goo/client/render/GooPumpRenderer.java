...
public class GooPumpRenderer extends TileEntityRenderer<GooPumpTile> {
    ...
    public void render(GooPumpTile tile, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLightIn, int combinedOverlayIn) {
        ...
        float totalGoo = tile.goo().getAmount();
        ...
        float percentage = tile.goo().getAmount() / totalGoo;
        ...
        toY = fromY + height;
        highestToY = toY;
        FluidCuboidHelper.renderScaledFluidCuboid(tile.goo(), matrixStack, builder, combinedLightIn, from.getX(), fromY, from.getZ(), to.getX(), toY, to.getZ());
        yOffset += height;
