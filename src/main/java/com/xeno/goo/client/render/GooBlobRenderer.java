...
public class GooBlobRenderer extends EntityRenderer<GooBlob>
{
    ...
    public void render(GooBlob entity, float entityYaw, float partialTicks, MatrixStack stack, IRenderTypeBuffer bufferType, int light)
    {
        ...
        TextureAtlasSprite sprite = Minecraft.getInstance().getModelManager().getAtlasTexture(PlayerContainer.LOCATION_BLOCKS_TEXTURE).getSprite(entity.onlyGoo().getFluid().getAttributes().getStillTexture());
...
//        }
    ...
    private boolean isBrightFluid(Fluid fluid, long gameTime)
    {
        return fluid.equals(Registry.MOLTEN_GOO.get()) || fluid.equals(Registry.ENERGETIC_GOO.get())
                || (fluid.equals(Registry.LOGIC_GOO.get()) && gameTime % 20 == 0);
    }
