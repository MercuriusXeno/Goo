...
public class GooEntityTags extends EntityTypeTagsProvider {
	...
	public GooEntityTags(DataGenerator g, ExistingFileHelper existingFileHelper) {
	...
	private void registerPrimordialInstantDeathEntities() {
		for(EntityType<?> immuneMob : primordialImmuneMobs) {
			this.getOrCreateBuilder(PRIMORDIAL_INSTANT_DEATH_IMMUNE_MOBS)
					.add(immuneMob);
		}
	}
	...
	private void registerPrimordialSpawnEggEntities() {
		...
		for(SpawnEggItem egg : eggs.collect(Collectors.toList())) {
			...
			this.getOrCreateBuilder(PRIMORDIAL_SPAWN_EGGS_ALLOWED)
	...
	private void registerWaterHatingMobs() {
		this.getOrCreateBuilder(WATER_HATING_MOBS)
	...
	private void registerColdHatingMobs() {
		this.getOrCreateBuilder(COLD_HATING_MOBS)
				.add(EntityType.BEE)
				.add(EntityType.BLAZE)
