...
class IGasketHolderTest {
    ...
    /** Minimal stub that inherits all defaults via GasketState.none(). */
    private static class StubHolder implements IGasketHolder {
        @Override public GasketState gasketState() { return GasketState.none(); }
    }
    ...
    @Test
    void supportsRole_none_falseForBothRoles() {
        StubHolder holder = new StubHolder();
        assertFalse(holder.supportsRole(GasketRole.RECEIVER));
        assertFalse(holder.supportsRole(GasketRole.TRANSMITTER));
    }
